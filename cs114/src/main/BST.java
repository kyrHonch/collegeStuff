...
class BST<K extends Comparable<K>, E> implements Dictionary<K,   E> {
  ...
  void rangehelp(BSTNode<K,E> root,ArrayList<E> ans,K low,K high){
    if(root==null){
      return ;
    }
    if(root.key.compareTo(low)>=0)
      rangehelp(root.left,ans,low,high);

    if(low.compareTo(root.key)<=0&&high.compareTo(root.key)>=0){
      ans.add((Integer) root.key, root.element());
    }
    if(root.key.compareTo(high)<=0)
      rangehelp(root.right,ans,low,high);
  }
}
